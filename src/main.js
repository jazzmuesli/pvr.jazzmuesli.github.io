...
import { STEPS_PER_DAY } from "./calc/types";
import { buildControls } from "./ui/controls";
import { DEFAULT_STATE, toSimConfig } from "./ui/state";
import { renderMonthlyChart, renderHourlyChart, renderLegend, } from "./ui/charts";
...
const monthlyHost = document.getElementById("monthly");
const hourlyHost = document.getElementById("hourly");
const legendHost = document.getElementById("legend");
const monthTitle = document.getElementById("month-title");
let selectedMonth = 6; // July
let rafPending = false;
function hourlyProfile(result, month) {
    const buckets = Array.from({ length: 24 }, () => ({ pv: 0, solarExport: 0, batteryExport: 0, charge: 0, priceSum: 0, n: 0 }));
    for (let i = 0; i < result.pv.length; i++) {
        if (monthForStep(i) !== month)
            continue;
...
function renderSummary(rev, sim) {
    const cards = [
...
function countNonPositive(sim) {
    let n = 0;
    for (let i = 0; i < sim.price.length; i++)
        if (sim.price[i] <= 0)
            n++;
    return n;
}
function recompute() {
    ...
    const monthly = rev.monthly.map((r) => ({
        month: r.month,
